...
class GraceIntegrationTest:
    ...
    def _init_solana_wallet_manager(self):
        ...
        solana_wallet_manager = SolanaWalletManager(
            user_profile_system=self.user_profile_system, gmgn_service=self.gmgn_service
        )
    ...
    def test_user_profile(self):
        ...
        try:
            ...
            if not create_result or "error" in create_result:
                ...
                return {
                    "success": False,
                    "message": f"Failed to create test user: {create_result}",
                }

            # Authenticate test user
            auth_result = self.user_profile_system.authenticate_user(
                username=self.test_user_id, password=self.test_user_password
            )
            ...
            if not auth_result or "error" in auth_result:
                ...
                return {
                    "success": False,
                    "message": f"Failed to authenticate test user: {auth_result}",
                }
            ...
            update_result = self.user_profile_system.update_user_data(
                user_id=self.test_user_id, data={"test_key": "test_value"}
            )
            ...
            if not update_result or "error" in update_result:
                ...
                return {
                    "success": False,
                    "message": f"Failed to update user data: {update_result}",
                }

            # Verify update
            updated_user_data = self.user_profile_system.get_user_data(
                self.test_user_id
            )

            if (
                not updated_user_data
                or "test_key" not in updated_user_data
                or updated_user_data["test_key"] != "test_value"
            ):
                logger.error("Failed to verify user data update")
                return {
                    "success": False,
                    "message": "Failed to verify user data update",
                }
            ...
            return {
                "success": False,
                "message": f"Error testing User Profile System: {str(e)}",
            }
    ...
    def test_wallet_integration(self):
        ...
        try:
            ...
            wallet_result = self.solana_wallet_manager.generate_internal_wallet(
                self.test_user_id
            )
            ...
            if not wallet_result or wallet_result.get("status") != "success":
                ...
                return {
                    "success": False,
                    "message": f"Failed to generate internal wallet: {wallet_result}",
                }
            ...
            # Get wallet balance
            balance_result = self.solana_wallet_manager.get_wallet_balance(
                self.test_user_id
            )
            ...
            nlp_result = self.solana_wallet_manager.process_natural_language_request(
                user_id=self.test_user_id, request="show my wallet balance"
            )
            ...
            return {
                "success": False,
                "message": f"Error testing Wallet Integration: {str(e)}",
            }
    ...
    def test_gmgn_service(self):
        ...
        try:
            ...
            nlp_result = self.gmgn_service.process_natural_language_request(
                request="What's the price of Solana?", user_id=self.test_user_id
            )
            ...
            return {
                "success": False,
                "message": f"Error testing GMGN Service: {str(e)}",
            }
    ...
    def test_transaction_confirmation(self):
        ...
        try:
            ...
            prepare_result = self.transaction_confirmation.prepare_transaction(
                user_id=self.test_user_id, transaction_type="swap", parameters=tx_params
            )

            if (
                not prepare_result
                or prepare_result.get("status") != "confirmation_required"
            ):
                logger.error(f"Failed to prepare transaction: {prepare_result}")
                return {
                    "success": False,
                    "message": f"Failed to prepare transaction: {prepare_result}",
                }
            ...
            pending_result = self.transaction_confirmation.get_pending_transactions(
                self.test_user_id
            )
            ...
            if not pending_result or pending_result.get("status") != "success":
                ...
                return {
                    "success": False,
                    "message": f"Failed to get pending transactions: {pending_result}",
                }

            # Cancel transaction
            cancel_result = self.transaction_confirmation.cancel_transaction(
                user_id=self.test_user_id, confirmation_id=confirmation_id
            )
            ...
            if not cancel_result or cancel_result.get("status") != "success":
                ...
                return {
                    "success": False,
                    "message": f"Failed to cancel transaction: {cancel_result}",
                }
            ...
            return {
                "success": True,
                "message": "Transaction Confirmation System test passed",
            }
            ...
            return {
                "success": False,
                "message": f"Error testing Transaction Confirmation System: {str(e)}",
            }
    ...
    def test_social_media_service(self):
        ...
        try:
            ...
            search_result = self.social_media_service.search_twitter(
                query="solana", search_type="recent", count=5
            )

            # Test user profile
            user_result = self.social_media_service.get_user_profile(username="solana")
            ...
            return {
                "success": False,
                "message": f"Error testing Social Media Service: {str(e)}",
            }
    ...
    def test_natural_language_processing(self):
        ...
        try:
            ...
            wallet_commands = ["show my wallet balance", "check my portfolio"]

            for command in wallet_commands:
                result = self.solana_wallet_manager.process_natural_language_request(
                    user_id=self.test_user_id, request=command
                )

            # Test price-related commands
            price_commands = ["What's the price of Solana?", "Show me SOL price chart"]

            for command in price_commands:
                result = self.gmgn_service.process_natural_language_request(
                    request=command, user_id=self.test_user_id
                )

            # Test transaction-related commands
            tx_commands = ["list pending transactions", "show my transactions"]
            ...
            for command in social_commands:
                result = self.social_media_service.execute_dynamic_function(
                    function_name="process_natural_language_query", query=command
                )

            logger.info("Natural Language Processing test passed")
            return {
                "success": True,
                "message": "Natural Language Processing test passed",
            }
            ...
            return {
                "success": False,
                "message": f"Error testing Natural Language Processing: {str(e)}",
            }
