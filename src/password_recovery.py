...
class PasswordRecoverySystem:
    ...
    def __init__(
        ...
        self.from_email = from_email or os.environ.get('FROM_EMAIL', 'noreply@grace.app')
    ...
    def send_recovery_email(self, email: str, recovery_token: str, reset_url: str) -> bool:
        """
        ...
        try:
            ...
            if self.smtp_username and self.smtp_password:
                logger.info("Logging in to SMTP server with username %s", self.smtp_username)
                server.login(self.smtp_username, self.smtp_password)
            else:
                logger.warning("No SMTP credentials provided, attempting to send without authentication")
            ...
            logger.error(f"Connection refused to SMTP server {self.smtp_host}:{self.smtp_port}")
            return False
    ...
    def initiate_recovery(self, email: str, user_id: str, reset_url: str, phone: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if os.environ.get('GRACE_ENV') == 'development':
            result['token'] = token
            logger.warning("Including recovery token in response (development mode only)")
       
        return result
   
    def complete_recovery(self, token: str, new_password: str, user_profile_system) -> Dict[str, Any]:
        """
