...
class DataWhizAgent(BaseAgent):
    ...
    def _handle_process_message(self, task: AgentTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if context_info and "completed_tasks" in context_info:
                for task in context_info["completed_tasks"]:
                    if task["type"] == "price_check" and "result" in task:
                        ...
                        result = task["result"]
                        response += f"I found some information about '{result['query']}': "
            ...
            if context_info and "active_topics" in context_info and context_info["active_topics"]:
                topics = [topic["name"] for topic in context_info["active_topics"][:2]]
                if topics and not response:  # Only add if we don't already have a response
            ...
            if not response:
                ...
                common_tokens = ["bitcoin", "btc", "ethereum", "eth", "solana", "sol", "dogecoin", "doge", 
                                "cardano", "ada", "ripple", "xrp", "bnb", "usdt", "tether", "usdc", "wif"]
                
                # Check if message contains any token names
                found_tokens = [token for token in common_tokens if token in message.lower()]
                ...
                elif ("trade" in message.lower() or "trading" in message.lower()) and found_tokens:
                ...
                elif ("price" in message.lower() or "prices" in message.lower() or "worth" in message.lower()) and found_tokens:
                    # Create a price check task for this token
                    token = found_tokens[0].upper()
                    try:
                        # Try to create a price check task
                        if self.api_services_manager:
                            price_info = self.api_services_manager.get_token_price(token)
    ...
    def _handle_get_community_pulse(self, task: AgentTask) -> Dict[str, Any]:
        ...
        # Get community pulse
        result = self.api_services_manager.twitter_service.get_community_pulse(community)
...
class TradingAgent(BaseAgent):
    ...
    def _handle_setup_auto_trading(self, task: AgentTask) -> Dict[str, Any]:
        ...
        if not user_id or risk_level is None or max_trade_size is None or stop_loss is None or take_profit is None:
            raise ValueError("user_id, risk_level, max_trade_size, stop_loss, and take_profit are required")
...
class CommunityTrackerAgent(BaseAgent):
    ...
    def _handle_discover_community_changes(self, task: AgentTask) -> Dict[str, Any]:
        ...
        result = self.api_services_manager.twitter_service.discover_community_changes(community)
...
class SmartRouter:
    ...
    def __init__(self, agents: Dict[str, BaseAgent], task_type_to_agent: Dict[str, Dict] = None):
        """
        ...
        """
        self.agents = agents
        self.agent_types = {agent.agent_type: agent_id for agent_id, agent in agents.items()}
    ...
    def route_task(self, task: AgentTask) -> bool:
        ...
        if task.target_agent and task.target_agent in self.agents:
            ...
            self.logger.info(f"Routed task {task.task_id} to specified agent {task.target_agent}")
            return True
        ...
        if task.task_type in self.task_type_to_agent:
            ...
            if AgentType.DATA_WHIZ in self.agent_types:
                ...
                self.logger.info(f"Routed task {task.task_id} of type {task.task_type} to agent {agent_id} (service proxy)")
                return True
        ...
        if task.task_type in self.task_type_mapping:
            ...
            if agent_type in self.agent_types:
                ...
                self.agents[agent_id].add_task(task)
                self.logger.info(f"Routed task {task.task_id} of type {task.task_type} to agent {agent_id}")
                return True
            else:
                self.logger.error(f"No agent of type {agent_type.value} available for task {task.task_id}")
                return False
        else:
            ...
            for agent_id, agent in self.agents.items():
                if task.task_type in agent.supported_task_types:
                    agent.add_task(task)
                    self.logger.info(f"Routed task {task.task_id} to agent {agent_id} based on supported task types")
                    return True
...
class SystemAgentManager:
    ...
    def __init__(
        ...
        self.max_tasks = self.config.get('max_tasks', 1000)  # Maximum number of tasks to keep
        self.task_retention_hours = self.config.get('task_retention_hours', 24)  # How long to keep completed tasks
        self.cleanup_interval = self.config.get('cleanup_interval', 100)  # Run cleanup every N tasks
    ...
    def register_agent_for_task_type(self, task_type: str, target_service: Any, priority: AgentPriority = AgentPriority.MEDIUM):
        """
        ...
        if not hasattr(target_service, 'process_task'):
            self.logger.warning(f"Service for task type {task_type} does not have a process_task method")
            return
    ...
    def get_task_status(self, task_id: str):
        ...
        if task_id not in self.tasks:
            return SimpleNamespace(status="unknown", result=None, error="Task not found")
    ...
    def create_task(
        ...
        if task_type in self.task_type_to_agent:
            ...
            try:
                ...
                self.logger.info(f"Processed task {task_id} with registered service for {task_type}")
                return task_id
            except Exception as e:
                self.tasks[task_id]['status'] = 'error'
                self.tasks[task_id]['error'] = str(e)
                self.logger.error(f"Error processing task {task_id} with registered service: {str(e)}")
    ...
    def _handle_trade_executed(self, task_result: Dict[str, Any]):
        ...
        # In a real implementation, this would trigger additional actions
        # For now, just log the event
        self.logger.info(f"Trade executed: {task_result.get('result', {}).get('trade_id')}")
    ...
    def cleanup_tasks(self, force: bool = False):
        ...
        if not force and self.task_id_counter - self.last_cleanup_count < self.cleanup_interval:
            return 0
        
        self.last_cleanup_count = self.task_id_counter
        self.logger.debug(f"Running task cleanup. Current task count: {len(self.tasks)}")
        ...
        if removed_count > 0:
            self.logger.info(f"Cleaned up {removed_count} tasks. Remaining: {len(self.tasks)}")
        
        return removed_count
    
    def schedule_task(self, task_type: str, content: Dict[str, Any], interval_seconds: int, priority: AgentPriority = AgentPriority.LOW):
        """
        ...
        self.logger.info(f"Scheduled task {task_id} of type {task_type} to run every {interval_seconds} seconds")
            ...
            try:
                ...
                for task_id, task_info in list(self.scheduled_tasks.items()):
                    if current_time - task_info['last_run'] >= task_info['interval']:
                        ...
                        self.logger.info(f"Running scheduled task {task_id} of type {task_info['task_type']}")
                        ...
                        if task_info['task_type'] in self.task_type_to_agent:
                            ...
                            try:
                                ...
                                self.logger.info(f"Processed scheduled task {task_id} with result: {result}")
                            except Exception as e:
                                self.logger.error(f"Error processing scheduled task {task_id}: {str(e)}")
                        else:
                            # Use the router if no direct service is available
                            if not self.router.route_task(task):
                                self.logger.error(f"Could not route scheduled task {task_id}")
    ...
    def process_task_sync(self, task: AgentTask, timeout: int = 30) -> Dict[str, Any]:
        ...
        Returns:
            Dict: Task result
        """
        self.logger.info(f"Processing task {task.task_id} of type {task.task_type} synchronously")
        ...
        try:
            ...
            if task.task_type in self.task_type_to_agent:
                ...
                try:
                    # Process the task with the registered service
                    result = service.process_task(task.content)
                    self.logger.info(f"Processed task {task.task_id} with registered service for {task.task_type}")
                    return result
                except Exception as e:
                    self.logger.error(f"Error processing task {task.task_id} with registered service: {str(e)}")
