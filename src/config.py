    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        for env_var, config_key in self.ENV_MAPPING.items():
            ...
            if env_value is not None:
                ...
                if config_key in ["short_term_memory_limit", "medium_term_memory_limit", 
                                 "transaction_confirmation_timeout", "ui_port"]:
                    try:
                        env_value = int(env_value)
                    except ValueError:
                        logger.warning(f"Could not convert {env_var}={env_value} to int, using default")
                        continue
                ...
                # Convert float values
                    ...
                    try:
                        ...
                        logger.warning(f"Could not convert {env_var}={env_value} to float, using default")
                        continue
