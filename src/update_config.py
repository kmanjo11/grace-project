...
class ConfigUpdater:
    ...
    def update_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            for match in class_init_matches:
                ...
                config_params = [
                    'data_dir', 'encryption_key', 'solana_rpc_url', 'solana_network',
                    'nitter_instance', 'gmgn_router_endpoint', 'gmgn_price_endpoint',
                ...
                for param in config_params:
                    ...
                    if re.search(param_pattern, init_params):
                        ...
                        init_params = re.sub(param_pattern, f'{param}: str = None', init_params)
                        
                        # Add config retrieval in method body
                        method_body_pattern = rf'def __init__\(self,\s*{re.escape(match.group(1))}\):(.*?)(?=\n\s*def|\Z)'
                        method_body_match = re.search(method_body_pattern, content, re.MULTILINE | re.DOTALL)
                        ...
                        if method_body_match:
                            ...
                            if re.search(param_set_pattern, method_body):
                                ...
                                if first_line_end != -1:
                                    method_body = method_body[:first_line_end+1] + config_line + method_body[first_line_end+1:]
                                else:
                                    method_body += config_line
                            
                            # Update method body in content
                            content = re.sub(method_body_pattern, f'def __init__(self, {init_params}):{method_body}', content, flags=re.MULTILINE | re.DOTALL)
                
                # Update content with modified init params
                content = re.sub(class_init_pattern, f'def __init__(self, {init_params}):', content, count=1)
    ...
    def update_all_files(self) -> Dict[str, Any]:
        ...
        logger.info(f"Updated {success_count}/{len(python_files)} files to use centralized config")
        
        if failed_files:
            logger.warning(f"Failed to update {len(failed_files)} files: {failed_files}")
