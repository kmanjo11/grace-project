...
async def init_redis():
    ...
    try:
        ...
        logger.warning("Falling back to in-memory storage for chat sessions. Data will not persist between restarts!")
        USING_FALLBACK = True
        return False
        ...
        if result.get('success'):
            ...
            try:
                ...
                if not USING_FALLBACK and redis:
                    ...
                    logger.info(f"Registering token for user {user_id} in active sessions")
                    ...
                    if not session_exists:
                        ...
                        await redis.hmset_dict(session_key, {
                    ...
                    if default_session_id not in IN_MEMORY_STORAGE['sessions'][user_id]:
                        IN_MEMORY_STORAGE['sessions'][user_id].append(default_session_id)
                        timestamp = datetime.utcnow().isoformat()
...
async def create_session():
    ...
    try:
        ...
        if USING_FALLBACK:
            ...
            logger.info(f"Using in-memory fallback to create session for user: {user_id}")
...
async def get_history(session_id):
    ...
    try:
        ...
        if USING_FALLBACK:
            # Use in-memory fallback
            logger.info(f"Using in-memory fallback to get chat history for session: {session_id}")
...
async def handle_chat_message_with_session(user_id, message, session_id=None):
    ...
    try:
        ...
        if not session_id:
            ...
            if USING_FALLBACK:
                # Use in-memory fallback
                logger.info(f"Using in-memory fallback to create session for message from user: {user_id}")
                # Initialize user's sessions list if it doesn't exist
                if user_id not in IN_MEMORY_STORAGE['sessions']:
                    IN_MEMORY_STORAGE['sessions'][user_id] = []
        ...
        if USING_FALLBACK:
            ...
            if session_id in IN_MEMORY_STORAGE['metadata']:
                ...
                if not topic or topic == 'New Chat':
                    ...
                    IN_MEMORY_STORAGE['metadata'][session_id]['name'] = new_topic  # For React frontend
        else:
            # Use Redis
            # Store user message
        ...
        return {'error': str(e), 'session_id': session_id, 'using_fallback': USING_FALLBACK}
