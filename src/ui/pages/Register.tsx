    ...
    <div className="flex min-h-screen items-center justify-center bg-black text-white">
      <div className="w-full max-w-sm rounded-xl bg-black/70 p-6 shadow-xl border border-red-700">
        ...
        <button
          ...
          className={`mt-4 w-full rounded py-2 text-white ${isSubmitting ? 'bg-red-800' : 'bg-red-600'}`}
        >
          {isSubmitting ? 'Creating Account...' : 'Register'}
        </button>
        <div className="mt-4 text-center text-sm text-gray-400">
          Already have an account? <a href="/login" className="hover:text-red-400">Login</a>
        </div>
      </div>
    </div>
  );
