...
async def login():
    ...
    try:
        ...
        expiry = JWT_EXPIRY * 30 if remember_me else JWT_EXPIRY  # 30 days if remember_me
        token = generate_jwt_token(user_id, expiry)
        
        # Clean up existing sessions for this user if they have too many
        user_sessions = [t for t, s in active_sessions.items() if s.get('user_id') == user_id]
...
async def logout():
    ...
    sessions_removed = 0
    sessions_to_remove = [t for t, s in active_sessions.items() if s.get('user_id') == user_id]
    for t in sessions_to_remove:
        del active_sessions[t]
        sessions_removed += 1
        
    logger.info(f"User logged out: {username} (user_id: {user_id}, sessions removed: {sessions_removed})")
