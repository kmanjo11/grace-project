...
from mango_v3_extension import MangoV3Extension  # Importing Mango V3 if needed

# Initialize the service with configuration
default_config = {
    'mango_v3': {'enabled': True, 'url': 'http://localhost:8000'}
}
...
def test_spot_trade():
    try:
        ...
        spot_result = service.place_limit_order(spot_market, spot_side, spot_price, spot_size, trade_type=spot_trade_type)
...
def test_leverage_trade():
    try:
        ...
        leverage_result = service.place_limit_order(leverage_market, leverage_side, leverage_price, leverage_size, trade_type=leverage_trade_type)
