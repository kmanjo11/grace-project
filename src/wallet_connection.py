...
class InternalWalletManager:
    ...
    def get_wallet_balance(self, public_key: str) -> Dict[str, float]:
        ...
        try:
            ...
            response = requests.post(self.solana_rpc_url, headers=headers, json=payload, timeout=10)
...
class WalletConnectionSystem:
    ...
    def create_internal_wallet(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.user_profile_system.update_user_profile(
                user_id,
                {'internal_wallet': wallet_data}
            )
            
            if not result.get('success', False):
                raise ValueError(f"Failed to update user profile: {result.get('error')}")
    ...
    def update_internal_wallet_balance(self, user_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            # Update balance
            updated_wallet = self.internal_wallet_manager.update_wallet_balance(wallet_data)
            
            # Update user profile
            result = self.user_profile_system.update_user_profile(
                user_id,
                {'internal_wallet': updated_wallet}
            )
    ...
    def complete_phantom_connection(self, user_id: str, session_id: str, wallet_address: str) -> Dict[str, Any]:
        """
        Complete Phantom wallet connection.
        ...
        try:
            # Complete connection
            wallet_data = self.phantom_connector.complete_connection(session_id, wallet_address)
            ...
            if not update_result.get('success', False):
                raise ValueError(f"Failed to update user profile: {update_result.get('error')}")
    ...
    def disconnect_phantom_wallet(self, user_id: str, wallet_address: str) -> bool:
        ...
        try:
            ...
            if 'phantom_wallets' in profile:
                profile['phantom_wallets'] = [
                    w for w in profile['phantom_wallets']
    ...
    def update_all_wallet_balances(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, wallet in enumerate(wallets['phantom_wallets']):
                balance = self.phantom_connector.get_wallet_balance(wallet['wallet_address'])
