...
class MemorySystem:
    ...
    def _get_user_collection(self, user_id: str):
        ...
        if collection_name not in self.user_collections:
            self.user_collections[collection_name] = self._get_or_create_collection(
                collection_name
            )
    ...
    def add_to_short_term(
        ...
        # Add to ChromaDB
        user_collection.add(ids=[memory_id], documents=[text], metadatas=[metadata])
        ...
        if user_id and self.user_profile_system:
            if not self.user_profile_system.is_authorized_user(user_id):
                raise PermissionError(
                    f"User {user_id} is not authorized for global memory updates"
                )
    ...
    def process_grace_learn_command(self, user_id: str, command: str) -> Dict[str, Any]:
        ...
        """
        if (
            not self.user_profile_system
            or not self.user_profile_system.is_authorized_user(user_id)
        ):
            raise PermissionError(
                f"User {user_id} is not authorized for !grace.learn commands"
            )
        ...
        if user_id and ("short_term" in memory_types or "medium_term" in memory_types):
            ...
            if user_results["documents"]:
                for i, doc in enumerate(user_results["documents"][0]):
                    if i < len(user_results["metadatas"][0]) and i < len(
                        ...
                        results.append(
                            {
                                ...
                                "relevance": 1.0
                                - min(user_results["distances"][0][i], 1.0),
        ...
        if "long_term" in memory_types:
            ...
            if global_results["documents"]:
                for i, doc in enumerate(global_results["documents"][0]):
                    if i < len(global_results["metadatas"][0]) and i < len(
                        ...
                        results.append(
                            {
                                ...
                                "relevance": 1.0
                                - min(global_results["distances"][0][i], 1.0),
    ...
    def process_task(self, task_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if task_type == "query_memory":
            ...
            if memory_type == "short_term":
                ...
                memory_id = self.add_to_long_term(
                    text, metadata=metadata, user_id=user_id
                )
            else:
                return {
                    "status": "error",
            ...
            return {
                "status": "success",
                "message": f"Added to {memory_type} memory",
                "memory_id": memory_id,
            }
    ...
    def _prune_memories_by_type(self, memory_type: str, cutoff_time: float):
        ...
        for collection_name, collection in self.user_collections.items():
            ...
            expired_memories = collection.query(
                query_texts=[""],
                where={"memory_type": memory_type, "timestamp": {"$lt": cutoff_time}},
                n_results=100,
            )
            ...
            if expired_memories["ids"] and expired_memories["ids"][0]:
                ...
                logger.info(
                    f"Pruned {len(expired_memories['ids'][0])} {memory_type} memories from {collection_name}"
                )

    def get_conversation_context(
        self, user_id: str, n_messages: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        results = user_collection.query(
            query_texts=[""], where={"memory_type": "short_term"}, n_results=n_messages
        )
    ...
    def merge_related_memories(self, threshold: float = 0.85):
        ...
        all_memories = self.global_collection.query(
            query_texts=[""], where={"memory_type": "long_term"}, n_results=1000
        )
        ...
        for i, doc in enumerate(all_memories["documents"][0]):
            if (
                i < len(all_memories["metadatas"][0])
                and "entity" in all_memories["metadatas"][0][i]
            ):
            ...
            for i in range(len(memories)):
                for j in range(i + 1, len(memories)):
                    ...
                    if (
                        ...
                        and similarity["distances"][0]
                        and similarity["distances"][0][0] < (1.0 - threshold)
                    ):
                        ...
                        self.add_to_long_term(
                            text=merged_text, entity=entity, metadata={"merged": True}
                        )

                        # Delete old memories
                        self.global_collection.delete(
                            ids=[memories[i]["id"], memories[j]["id"]]
                        )
        ...
        if task_type == "query_memory":
            ...
            if memory_type == "short_term":
                ...
                if not entity:
                    return {
                        "status": "error",
                        "message": "Entity is required for long-term memory",
                    }
...
class MemoryBridgeSystem:
    ...
    def process_message(
        self, message: str, user_id: str, is_user: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if is_user and message.strip().startswith("!grace.learn"):
            try:
                result = self.memory_system.process_grace_learn_command(
                    user_id, message
                )
        ...
        memory_id = self.memory_system.add_to_short_term(
            user_id=user_id, text=message, metadata=metadata
        )
        ...
        return {"type": "message", "memory_id": memory_id, "entities": list(entities)}
    ...
    def get_relevant_context(
        self, query: str, user_id: str, max_items: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        for entity in entities:
            entity_results.extend(
                self.memory_system.query_by_entity(entity, n_results=3)
            )
    ...
    def update_entity_relationships(self):
        ...
        for collection_name, collection in self.memory_system.user_collections.items():
            ...
            recent_messages = collection.query(
                query_texts=[""], where={"memory_type": "short_term"}, n_results=100
            )

            # Sort manually by timestamp if needed
            if (
                recent_messages["documents"]
                and recent_messages["documents"][0]
                and len(recent_messages["metadatas"]) > 0
            ):
                # Create a list of (doc, metadata, id) tuples for sorting
                items = []
                for i, doc in enumerate(recent_messages["documents"][0]):
                    if (
                        i < len(recent_messages["metadatas"][0])
                        and "timestamp" in recent_messages["metadatas"][0][i]
                    ):
                        items.append(
                            (
                                doc,
                                recent_messages["metadatas"][0][i],
                                recent_messages["ids"][0][i],
                            )
                        )
            ...
            for i, doc in enumerate(recent_messages["documents"][0]):
                if (
                    i < len(recent_messages["metadatas"][0])
                    and "entities" in recent_messages["metadatas"][0][i]
                ):
    ...
    def get_related_entities(
        self, entity: str, max_entities: int = 5
    ) -> List[Tuple[str, int]]:
    ...
    def generate_context_for_prompt(
        self, query: str, user_id: str, max_items: int = 5
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    memory_bridge.process_message(
        "Hello, I'm interested in learning about Bitcoin.", "user_1", True
    )
    memory_bridge.process_message(
        "Bitcoin is a decentralized cryptocurrency created by Satoshi Nakamoto.",
        "user_1",
        False,
    )
    ...
    prompt_context = memory_bridge.generate_context_for_prompt(
        "Tell me about Bitcoin", "user_1"
    )
    print("\nPrompt context:\n", prompt_context)
