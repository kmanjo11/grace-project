...
class MangoWalletIntegrator:
    ...
    def link_wallet_silently(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not wallet_result.get("status") == "success":
                return {
                    "success": False,
                    "message": f"Failed to get/create wallet: {wallet_result.get('message')}",
                }
            ...
            link_result = self.mango_v3_client.link_wallet(
                wallet_address=public_key, user_id=user_id
            )
    ...
    def enhance_trade_execution(self, trade_func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if (
                not mango_balance["success"]
                or mango_balance.get("balance", 0) < required_amount
            ):
                ...
                if not internal_balance["success"]:
                    return {
                        "success": False,
                        "message": "Failed to check internal balance",
                    }

                # Calculate needed allocation
                current_mango_balance = (
                    mango_balance.get("balance", 0) if mango_balance["success"] else 0
                )
                ...
                allocation_result = self.allocate_funds(
                    user_id=user_id, amount=needed_amount, asset=asset
                )
