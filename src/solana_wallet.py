...
class SolanaWalletManager:
    ...
    def __init__(
        ...
        self.phantom_callback_path = os.environ.get("PHANTOM_CALLBACK_PATH", "/phantom/callback")
    ...
    def generate_internal_wallet(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.wallet_connection_system:
                ...
                logger.info(f"Using WalletConnectionSystem to generate wallet for user {user_id}")
                wallet_data = self.wallet_connection_system.create_internal_wallet(user_id)
                return {
                    "status": "success",
                    "message": "Internal wallet created successfully",
                    "wallet_address": wallet_data.get("public_key") or wallet_data.get("address"),
                    "wallet_data": wallet_data
                }
            
            # Fallback if WalletConnectionSystem is not available
            logger.warning("WalletConnectionSystem not available, using fallback method")
            ...
            if self.secure_data_manager:
                ...
                encrypted_private_key = self.secure_data_manager.encrypt(private_key_bytes)
            else:
                # Base64 encode private key if no secure manager
                private_key_bytes = keypair["private_key"]
                encrypted_private_key = base64.b64encode(private_key_bytes).decode('utf-8')
            ...
            self.user_profile_system.update_user_data(user_id, {"internal_wallet": wallet_data})
    ...
    def connect_phantom_wallet(self, user_id: str, phantom_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not phantom_address or len(phantom_address) < 32:
                return {
                    "status": "error",
                    "message": "Invalid Phantom wallet address"
                }
            ...
            self.user_profile_system.update_user_data(user_id, {"phantom_wallet": phantom_data})
            
            logger.info(f"Connected Phantom wallet for user {user_id}: {phantom_address}")
    ...
    def disconnect_phantom_wallet(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Disconnected Phantom wallet for user {user_id}: {phantom_address}")
    ...
    def get_wallet_balance(self, user_id: str, wallet_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.wallet_connection_system:
                logger.info(f"Using WalletConnectionSystem to get wallet balances for user {user_id}")
                try:
                    # Get all wallets and update their balances
                    wallets = self.wallet_connection_system.update_all_wallet_balances(user_id)
                    ...
                    if (wallet_type is None or wallet_type == "internal") and wallets.get("internal_wallet"):
                        internal_wallet = wallets["internal_wallet"]
                        wallet_info = {
                            "type": "internal",
                            "address": internal_wallet.get("public_key") or internal_wallet.get("address"),
            ...
            if wallet_type is None or wallet_type == "phantom":
                ...
                for phantom_wallet in phantom_wallets:
                    address = phantom_wallet.get("wallet_address") or phantom_wallet.get("address")
    ...
    def _get_solana_balance(self, wallet_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"RPC request failed: {response.status_code} - {response.text}")
                return {
                    "status": "error",
    ...
    def execute_transaction(
        ...
        try:
            ...
            logger.info(f"Using GMGN trade endpoint: {self.gmgn_service.trade_endpoint}")
    ...
    def process_task(self, task_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not user_id:
            return {"status": "error", "message": "User ID is required for wallet operations"}
        ...
        if task_type == "get_wallet_balance":
            ...
            if not phantom_address:
                return {"status": "error", "message": "Phantom wallet address is required"}
            return self.connect_phantom_wallet(user_id, phantom_address)
